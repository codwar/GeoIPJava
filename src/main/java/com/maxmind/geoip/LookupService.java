	...
	public synchronized Region getRegion(long ipnum) {
		...
		if (databaseType == DatabaseInfo.REGION_EDITION_REV0) {
			...
			if (seek_region >= 1000) {
				...
				record.setCountryName("United States");
				ch[0] = (char) (((seek_region - 1000) / 26) + 65);
				ch[1] = (char) (((seek_region - 1000) % 26) + 65);
				record.setRegion(new String(ch));
			} else {
				record.setCountryCode(countryCode[seek_region]);
				record.setCountryName(countryName[seek_region]);
				record.setRegion("");
			}
		} else if (databaseType == DatabaseInfo.REGION_EDITION_REV1) {
			...
			if (seek_region < US_OFFSET) {
				...
				record.setCountryCode("CA");
				record.setCountryName("Canada");
				ch[0] = (char) (((seek_region - CANADA_OFFSET) / 26) + 65);
				ch[1] = (char) (((seek_region - CANADA_OFFSET) % 26) + 65);
				record.setRegion(new String(ch));
			} else {
				record.setCountryCode(countryCode[(seek_region - WORLD_OFFSET) / FIPS_RANGE]);
				record.setCountryName(countryName[(seek_region - WORLD_OFFSET) / FIPS_RANGE]);
				record.setRegion("");
			}
		}
		return record;
